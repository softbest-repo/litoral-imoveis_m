<?php
	...
	if($_GET['codUsuario'] != ""){
		...
		function padronizarNumero($numero) {
			...
			if (strlen($numero) >= 10) {
				...
				if (strlen($telefone) === 9 && $telefone[0] === '9') {
					// Se o DDD for 11 ou 19, mantém o 9
					if ($ddd === '11' || $ddd === '19') {
						return $ddd . $telefone;
					}
		...
		$sqlConfereU = "SELECT U.* FROM usuarios U INNER JOIN usuariosLogins UL ON U.codUsuario = UL.codUsuario AND DATE(UL.dataUsuarioLogin) = CURDATE() WHERE  U.codUsuario = ".$_GET['codUsuario']." and U.statusUsuario = 'T' and U.tipoUsuario = 'C' ORDER BY U.codUsuario ASC";
