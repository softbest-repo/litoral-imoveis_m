<?php
	...
	if(isset($_GET['ref'])){
		...
		if($dadosConfereUltimoLead['codUsuario'] == $dadosUsuarioLead['codUsuario']){
			...
			if($dadosLead['statusLead'] == "T" || $dadosLead['statusLead'] == "R"){		
				...
				if($response === false) {
					...
					if($resultUpdate == 1){						
						$sqlInsereNegociacoes = "INSERT INTO negociacoes VALUES(0, 73, ".$dadosUsuarioLead['codUsuario'].", 0, 0, ".$dadosUsuarioLead['codUsuario'].", 0, '".$dadosConversa['nome']."', '".date('Y-m-d')."', '".date('Y-m-d')."', '".date('Y-m-d')."', '".date('H-i-s')."', '".formatarNumeroTelefone($dadosConversa['numero'])."', 'N', '0.00', '0.00', 'C', 'EA', '', '', '".$dadosMensagemLead['mensagem']."', 'WH', 'T')";
