<?php
	...
	while($dadosConfere = $resultConfere->fetch_assoc()){
		...
		if($dadosRecorrencia['codConversa'] != ""){
			...
			if($dadosReLead['statusLead'] == "F"){
				...
				if($dadosUsuario['codUsuario'] != ""){
					...
					$sqlLog = "INSERT INTO logs VALUES(0, 'Interage', 0, 'Entrou R', '".date('Y-m-d H:i:s')."')";
					...
					$sqlLog = "INSERT INTO logs VALUES(0, 'Interage', 0, 'Entrou T Recorrente', '".date('Y-m-d H:i:s')."')";
					$resultLog = $conn->query($sqlLog);
				...
				$sqlLog = "INSERT INTO logs VALUES(0, 'Interage', 0, 'Entrou F', '".date('Y-m-d H:i:s')."')";
				$resultLog = $conn->query($sqlLog);
			...
			$sqlLog = "INSERT INTO logs VALUES(0, 'Interage', 0, 'Entrou T', '".date('Y-m-d H:i:s')."')";
			$resultLog = $conn->query($sqlLog);
		...
		$phone_number_id = $phone_number_id_interage[$dadosConfere['idWa']];
		$token = $tokenInterage[$dadosConfere['idWa']];
